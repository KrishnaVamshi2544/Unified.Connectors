...
namespace Unified.Connectors.Services
{
    public class HoldService
    {
        ...
        public HoldService(UnifiedDbContext context)
        {
            _unifiedDbContext = context;
        }

        // Asynchronous call to create a hold
        public async Task<HoldStats> CreateHoldAsync(Guid jobId, UserRequestModel userRequestModel)
        {
            try
            {
                //Get Exchange Configuration
                var configuration = new CompanyConfig().GetConfig(_unifiedDbContext, userRequestModel.CompanyId, userRequestModel.ClientId);
        ...
        // Helper to update job status and metadata
        private async Task UpdateJobStatus(Guid jobId, string status, int? count = null, long? size = null)
        {
            var job = await _unifiedDbContext.JobQueues.FirstOrDefaultAsync(j => j.JobId == jobId);
            if (job != null)
            {
                job.Status = status;
                //job.Count = count ?? job.Count;
                //job.Size = size ?? job.Size;
                job.ModificationDate = DateTime.UtcNow;

                await _unifiedDbContext.SaveChangesAsync();
            }
        }
    }
}
